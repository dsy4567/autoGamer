...
- `<dataDir>/scriptData/<id>/config.override.js` 脚本用户配置（覆盖 scripts/<id>/config.default.js，运行时生成）
...
- 【重要】在没有用户的明确指令的情况下，不要理会 `~/.autoGamer/` 目录下的文件，这是运行时副本而非源代码。
- 【重要】操作文件前，务必检查文件是否在 `~/.autoGamer/` 目录下，避免操作错误文件。
...
- 如果需要运行浏览器进行测试，除非用户明确说明，否则应该指导用户手动操作，而不是自己直接运行。

### 增减依赖

- 允许按需增加或减少依赖，但必须经过用户同意
- 尽可能减少依赖，保持项目轻量程度
